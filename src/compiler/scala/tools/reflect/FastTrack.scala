...
trait FastTrack {
  ...
  lazy val fastTrack: Map[Symbol, FastTrackEntry] = {
    ...
    TraversableLike_map bindTo { 
      // TODO(geokollias): List(1,2,3).macroMap(_+1) is broken
      ...
      case (c, app@Apply(Apply(TypeApply(Select(prefix, _), List(outElemTT, outCollTT)), List(f1@Block(_, Function(List(ValDef(_, _, inElemTT, _)), expr)))), List(bf))) => {
        c.macro_TraversableLike_macroMap(f1, inElemTT.tpe, expr.tpe, prefix.tpe, outCollTT.tpe, bf)
      }
    }
